...
Rectangular_Capacity_7000 <- read_csv("C:/Users/DvdRe/Desktop/TFG/DataSets/Rectangular Capacity7000.csv")
View(Rectangular_Capacity_7000)
names (Rectangular_Capacity_7000) = c("Time","MCS","Time","RCVD_SINR_ENB","Time","Capacity","Time","Server_throughput")

throughput<-Rectangular_Capacity_7000[1:1915,7:8]
snrENB<-Rectangular_Capacity_7000[1:246784,3:4]
capacity<-Rectangular_Capacity_7000[,5:6]
mcs<-Rectangular_Capacity_7000[,1:2]
...
conf_bound_snr[1]<-mean_snr-stddev_snr/sqrt(tam_snr[1])*conf_intervals[1]
conf_bound_snr[2]<-mean_snr+stddev_snr/sqrt(tam_snr[1])*conf_intervals[1]
conf_bound_snr[3]<-mean_snr-stddev_snr/sqrt(tam_snr[1])*conf_intervals[2]
conf_bound_snr[4]<-mean_snr+stddev_snr/sqrt(tam_snr[1])*conf_intervals[2]
conf_bound_snr[5]<-mean_snr-stddev_snr/sqrt(tam_snr[1])*conf_intervals[3]
conf_bound_snr[6]<-mean_snr+stddev_snr/sqrt(tam_snr[1])*conf_intervals[3]
...
conf_bound_thr[1]<-mean_thr-stddev_thr/sqrt(tam_thr[1])*conf_intervals[1]/1000
conf_bound_thr[2]<-mean_thr+stddev_thr/sqrt(tam_thr[1])*conf_intervals[1]/1000
conf_bound_thr[3]<-mean_thr-stddev_thr/sqrt(tam_thr[1])*conf_intervals[2]/1000
conf_bound_thr[4]<-mean_thr+stddev_thr/sqrt(tam_thr[1])*conf_intervals[2]/1000
conf_bound_thr[5]<-mean_thr-stddev_thr/sqrt(tam_thr[1])*conf_intervals[3]/1000
conf_bound_thr[6]<-mean_thr+stddev_thr/sqrt(tam_thr[1])*conf_intervals[3]/1000

throughputFINAL$Server_throughput<-throughputFINAL$Server_throughput/1000

p1<-ggplot(data=throughputFINAL, aes(x=Time, y=Server_throughput, group=2)) +
  geom_smooth(method = "FFTSmooth", method.args = list(n = 0.2), color = "pink", size=2)+stat_smooth(color="black")+
  geom_point(color="green")+scale_color_brewer(palette="Dark2")+
  labs(x = "Time [s]",y = "Throughput [KBps]")+
  ggtitle ("Throughput values-UE Rectangular Movement")+
  scale_y_continuous(trans = "log10")
  #scale_y_continuous(limit = c(0,5000), breaks=seq(100, 5000, 100))

p2<-ggplot(data=snrENBFINAL, aes(x=Time, y=RCVD_SINR_ENB, group=2)) +
  geom_smooth(method = "FFTSmooth", method.args = list(n = 0.2), color = "purple", size=2)+stat_smooth(color="white")+
  geom_point(color="black")+scale_color_brewer(palette="Dark2")+
  labs(x = "Time [s]",y = "SINR [dB]")+
  ggtitle ("SINR measured at the eNB-UE Rectangular Movement")

nb.cols <- 18
mycolors <- colorRampPalette(brewer.pal(8, "Set2"))(nb.cols)

p3<-ggplot(data=cap_mcs_FINAL, aes(x=Time, y=Capacity, color=MCS, group=1)) +
  geom_smooth(method = "FFTSmooth", method.args = list(n = 0.2),color = "darkolivegreen3", size=1)+stat_smooth(color="darkmagenta")+
  geom_point()+geom_hline(yintercept=7, size=2,linetype="dotted")+
  labs(x = "Time [s]",y = "Comp. Capacity [Kbit-iter/s prb]")+scale_y_continuous( breaks = seq(0,10,1), limits=c(0,10))+
  ggtitle ("Computational Capacity-Rectangular Movement")+scale_fill_manual(values = mycolors)+
