...
Stationary_Capacity_7000 <- read_csv("C:/Users/DvdRe/Desktop/TFG/DataSets/Stationary 2 Capacity 7000.csv")
View(Stationary_Capacity_7000)
names (Stationary_Capacity_7000) =  c("Time","Server_throughput","Time","Capacity","Time","RCVD_SINR_ENB","Time","Server_throughput")


throughput<-Stationary_Capacity_7000[1:1915,1:2]
snrENB<-Stationary_Capacity_7000[1:246784,5:6]
capacity<-Stationary_Capacity_7000[,3:4]
mcs<-Stationary_Capacity_7000[,7:8]
...
cap_mcs_FINAL$MCS<-as.factor(cap_mcs_FINAL$MCS)
...
p1<-ggplot(data=throughputFINAL, aes(x=Time, y=Server_throughput, group=2)) +
  ...
  geom_point(color="red")+scale_color_brewer(palette="Dark2")+
  labs(x = "Time [s]",y = "Throughput [KBps]")+
  ggtitle ("Throughput values-UE Stationary ")+
...
p2<-ggplot(data=snrENBFINAL, aes(x=Time, y=RCVD_SINR_ENB, group=2)) +
  ...
  ggtitle ("SINR measured at the eNB-Stationary UE")

nb.cols <- 18
mycolors <- colorRampPalette(brewer.pal(8, "Set2"))(nb.cols)

p3<-ggplot(data=cap_mcs_FINAL, aes(x=Time, y=Capacity, color=MCS ,group=2)) +
  geom_smooth(method = "FFTSmooth", method.args = list(n = 0.2),color = "darkgoldenrod2", size=2)+stat_smooth(color="chartreuse4")+
  geom_point()+scale_color_brewer(palette="Dark2")+geom_hline(yintercept=7, size=2,linetype="dotted")+
  labs(x = "Time [s]",y = "Comp. Capacity [Kbit-iter/s prb]")+scale_y_continuous( breaks = seq(0,10,1), limits=c(0,10))+
  ggtitle ("Computational Capacity-Stationary UE")+scale_fill_manual(values = mycolors)+
