...
ConstantSpeed_7000 <- read_csv("C:/Users/DvdRe/Desktop/TFG/DataSets/ConstantSpeed Capacity 7000.csv")
View(ConstantSpeed_7000)
names (ConstantSpeed_7000) =  c("Time","MCS","Time","RCVD_SINR_ENB","Time","Capacity","Time","Server_throughput")

throughput<-ConstantSpeed_7000[1:2743,7:8]
snrENB<-ConstantSpeed_7000[1:246784,3:4]
capacity<-ConstantSpeed_7000[,5:6]
mcs<-ConstantSpeed_7000[,1:2]
...
p1<-ggplot(data=throughputFINAL, aes(x=Time, y=Server_throughput, group=1)) +
  geom_smooth(method = "FFTSmooth", method.args = list(n = 0.2),color = "pink", size=2)+stat_smooth(color="white")+
  geom_point(color="black")+scale_color_brewer(palette="Dark2")+
  labs(x = "Time [s]",y = "Throughput [KBps]")+
  ggtitle ("Throughput values-Constant Speed Movement")+
  scale_y_continuous(trans = "log10")

p2<-ggplot(data=snrENBFINAL, aes(x=Time, y=RCVD_SINR_ENB, group=2)) +
  geom_smooth(method = "FFTSmooth", method.args = list(n = 0.2),color = "green", size=2)+stat_smooth(color="yellow")+
  geom_point(color="brown")+scale_color_brewer(palette="Dark2")+
  labs(x = "Time [s]",y = "SINR [dB]")+
  ggtitle ("SINR measured at the eNB-Constant Speed Movement")

nb.cols <- 18
mycolors <- colorRampPalette(brewer.pal(8, "Set2"))(nb.cols)

p3<-ggplot(data=cap_mcs_FINAL, aes(x=Time, y=Capacity, color=MCS,group=1)) +
  geom_smooth(method = "FFTSmooth", method.args = list(n = 0.2),color = "red", size=1)+stat_smooth(color="black")+
  geom_point()+
  geom_hline(yintercept=7, size=2,linetype="dashed")+
  labs(x = "Time [s]",y = "Comp. Capacity [Kbit-iter/s prb]")+scale_y_continuous( breaks = seq(0,10,1), limits=c(0,10))+
  ggtitle ("Computational Capacity-Constant Speed Movement")+scale_fill_manual(values = mycolors)+
